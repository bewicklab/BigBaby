...
dd[,2][which(dd[,2]=='Paedomorphwinter')]<-'Paedomorph'
...
Pphylo<-prune_samples(sample_data(rcutMphylo)$StypeS %in% c('Paedomorphwinter'),rcutMphylo)
Pphylobig<-prune_samples(sample_data(Pphylo)$svl>51,Pphylo)
Pphylosmall<-prune_samples(sample_data(Pphylo)$svl<52,Pphylo)
...
df$stype<-factor(df$stype,levels=c('Larvae','Paedomorphwinter','Metamorph'))
...
p_maxis_4<-p_maxis_4+scale_fill_manual(values=c("seagreen2", 'royalblue1' ,"coral4"))
...
#Choose the violin colors for each group
p_paxis_4<-p_paxis_4+scale_fill_manual(values=c("seagreen2", 'royalblue1' ,"coral4"))
#Add boxplots inside the violins
