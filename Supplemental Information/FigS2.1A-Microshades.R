...
sample_data(rMphylo)$StypeS[which(sample_data(rMphylo)$StypeS=='Paedomorphwinter')]<-'w_Paedomorphs'
...
plot_diff2 <- plot2 + scale_y_continuous(labels = scales::percent, expand = expansion(0)) +
  ...
  facet_grid(~fct_relevel(StypeS,'Metamorphs','w_Paedomorphs','Larvae'), scale="free_x", space = "free_x") +
